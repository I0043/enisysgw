...
module Gw::RumiHelper
  ...
  def truncate_group_names(groups)
    names = [groups.first]
    names << t("rumi.label.other") + "（#{groups.count - 1}）" if groups.count >= 2
    names.join(" ").html_safe
  end

  def select_prop_group_tree(partition=nil)
    group_lists = []
    roots = Gw::PropGroup.where("parent_id=1 and state='public' and id > 1").order("sort_no, id")
  ...
  def get_prop_group_childs(group_lists, parent)
    c_lists = group_lists
    childs = Gw::PropGroup.where("state='public' and parent_id=#{parent.id}").order("sort_no, id")
  ...
  def select_prop_type(all=nil)
    type_lists = []
    type_lists << [t("rumi.search.all"),0] if all == 'all'
