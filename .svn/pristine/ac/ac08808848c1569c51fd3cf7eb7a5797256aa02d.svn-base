# -*- encoding: utf-8 -*-
class Doclibrary::Doc < Gw::Database
  ...
  has_many :reminders, -> {where(category: 'doclibrary')}, foreign_key: :item_id, class_name: 'Gw::Reminder',
      dependent: :destroy

  validates_presence_of :state
  after_validation :validate_edit_start, :validate_title, :validate_category1_id
  after_save :check_digit, :title_update_save, :count_doc_record
  ...
  def validate_title
    if self.title.blank?
      errors.add :title, I18n.t("rumi.doclibrary.message.validate_title") unless self.form_name == 'form002'
    end unless self.state == 'preparation'

    if self.section_code.blank?
        errors.add :section_code, I18n.t("rumi.doclibrary.message.validate_section_code")
    end unless self.form_name == 'form002' unless self.state == 'preparation'

    if self.category1_id.blank?
        errors.add :category1_id, I18n.t("rumi.doclibrary.message.validate_section_code") unless self.form_name == 'form002'
    end unless self.state == 'preparation'
  end
  ...
  def validate_category1_id
    unless self.category1_id.blank?
      ...
      if folder.blank?
        # フォルダが見つからなかった場合
        errors.add :category1_id, I18n.t("rumi.doclibrary.message.validate_category1_id") unless self.form_name == 'form002'
      end unless self.state == 'preparation'

      unless folder.present? && folder.admin_user?
        # フォルダへの管理権限がなかった場合
        errors.add :category1_id, I18n.t("rumi.doclibrary.message.validate_category_admin") unless self.form_name == 'form002'
  ...
  def public_path
    ...
    Core.public_path + content.public_uri + _name + '/index.html'
  end
  ...
  def self.get_creator_condition(value, column, division_id_column)
    cond = Condition.new
  ...
  def self.get_date_condition(value, column, option={:is_term_start => true})
    cond = Condition.new
    if option[:is_term_start]
      # 引数valueが期間開始の日付（下限値）の場合
      start_date = value.to_s.to_time.strftime("%Y-%m-%d")
      quote_string = connection.quote_string(start_date).gsub(/([_%])/, '\\\\\1')
  ...
  def parent_children_size_update(item, diff_count, state_flag=nil)
    ...
    parent = Doclibrary::GroupFolder.find_by(id: item.parent_id)
  ...
  def self.update_group_folder_children_size
    ...
    items = Doclibrary::GroupFolder.where("level_no > 1")
                                  .order('level_no DESC')
                                  .select('level_no').group('level_no')
  ...
  def self.update_total_chilldren_size(level_no, start_level_no)
    items = Doclibrary::GroupFolder.where(level_no: level_no)
                                   .order('parent_id ,code')
    parent_id = 0
    total_children_count = 0
    for up_item in items
      unless parent_id == up_item.parent_id
        item = Doclibrary::GroupFolder.find_by(id: parent_id)
        unless item.blank?
