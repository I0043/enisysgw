...
class Gwbbs::Script::Task
  ...
  def self.delete
    ...
    items = Gwbbs::Itemdelete.where(content_id: 1)
  ...
  def self.make_target_record(limit_date)
    Gwbbs::Itemdelete.where("content_id = 1").delete_all
    item = Gwbbs::Control
    items = item.where(limit_date: 'use').order('sort_no, id')
    for @title in items
      begin
...
#        dump "#{self} : 異常終了 : #{ex.message}"
  ...
  def self.destroy_record(id, limit)
    @title = Gwbbs::Control.find_by(id: id)
    unless @title.blank?
      @img_path = "public/_common/modules/#{@title.system_name}/"   #画像path指定
      item = Gwbbs::Doc
      @items = item.where(state: 'preparation')
      ...
      @items = item.where("state != 'preparation'")
                     .where("expiry_date < ?", limit.strftime("%Y-%m-%d") + ' 00:00:00')
      del_count = 0
      for @item in @items
        destroy_comments
        #destroy_image_files
