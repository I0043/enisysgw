...
class System::Group < ActiveRecord::Base
  ...
  has_many :enabled_children  , -> {where(state: 'enabled').order(:sort_no)}, :foreign_key => :parent_id, :class_name => 'System::Group'
  ...
  validates_each :state do |record, attr, value|
    if value.present?
      record.errors.add attr, I18n.t("rumi.error.group.parent_disable") if !record.parent.blank? && record.parent.state == "disabled" && record.state == "enabled"
    end
  end

  validates_each :end_at do |record, attr, value|
    if value.present?
      record.errors.add attr, I18n.t("rumi.error.group.enable_end_at_blank") if record.state == "enabled"
      record.errors.add attr, I18n.t("rumi.error.group.after_start_at") if Time.local(value.year, value.month, value.day, 0, 0, 0) < Time.local(record.start_at.year, record.start_at.month, record.start_at.day, 0, 0, 0)
      record.errors.add attr, I18n.t("rumi.error.group.before_today") if Time.local(value.year, value.month, value.day, 0, 0, 0) > Time.local(Time.now.year, Time.now.month, Time.now.day, 0, 0, 0)
    end
  end
  ...
  def save_group_history
    group_history = System::GroupHistory.find_by(id: self.id)
  ...
  def self.get_level2_groups
    ...
    groups = group.order(order).where(cond)
    return groups
  end
  ...
  def self.select_dd_group(day=nil,level=nil,parent_id=nil,all=nil)
    day   = Time.now     if day==nil
    dd_lists = []
    dd_lists << [I18n.t("rumi.select.all"), 0] if all == 'all'
    if parent_id ==nil
      if level==nil
      ...
      groups = System::Group.where(g_cond)
      groups.each do |g|
  ...
  def self.select_dd_tree2(all=nil)
    dd_lists = []
    dd_lists << [I18n.t("rumi.select.all"), 0] if all == 'all'
    roots = System::Group.where("level_no=2 and state='enabled'").order('sort_no')
    roots.each do |r|
      dd_lists << ['('+r.code+')'+r.name,r.id]
      dd_lists = System::Group.get_childs(dd_lists,r)
    end
  ...
  def self.get_group_select(all=nil, prefix='', options={})
    selects = []
    selects << [I18n.t("rumi.select.all"), 0] if all=='all'
    selects << [I18n.t("rumi.select.no_limit"),0] if all=='nolimit'
    cond = ''
  ...
  def ldaps
    [[I18n.t("rumi.state.synchro"), '1'], [I18n.t("rumi.state.asynchro"), '0']]
  end
end
