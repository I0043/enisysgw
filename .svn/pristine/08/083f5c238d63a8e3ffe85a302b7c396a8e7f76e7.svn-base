...
class Doclibrary::Control < Gw::Database
  ...
  has_many :docs, :foreign_key => :title_id, :class_name => 'Doclibrary::Doc', :dependent => :destroy
  has_many :folders, :foreign_key => :title_id, :class_name => 'Doclibrary::Folder', :dependent => :destroy
  has_many :group_folders, :foreign_key => :title_id, :class_name => 'Doclibrary::GroupFolder', :dependent => :destroy
  has_many :files, :foreign_key => :title_id, :class_name => 'Doclibrary::File'
  ...
  after_create :set_category_folder_root
  ...
  def save_admingrps
    unless self.admingrps_json.blank?
      Doclibrary::Adm.where("title_id=#{self.id}").destroy_all
  ...
  def save_readers
    unless self.readers_json.blank?
      Doclibrary::Role.where("title_id=#{self.id} and role_code = 'r'").destroy_all
  ...
  def validate_params
    ...
    unless error_users.blank?
      ...
      errors.add(:adms_json, I18n.t('rumi.doclibrary.message.invalid_user', error_user: error_user))
    end

    error_users = validate_users(self.sureaders_json)
    unless error_users.blank?
      error_user = error_users.join(", ")
      errors.add(:sureaders_json, I18n.t('rumi.doclibrary.message.invalid_user', error_user: error_user))
    end
  end
  ...
  def set_category_folder_root
    folder = Doclibrary::Folder.where(title_id: self.id, level_no: 1).first
    unless folder.blank?
      Doclibrary::Folder.update(folder.id,
        :updated_at => Time.now,
        :_acl_create_skip => true ,
        :name => self.category1_name
      )
    else
      folder = Doclibrary::Folder.new(
        :state => 'public',
        :title_id => self.id,
        :parent_id => nil,
        :sort_no => 0,
  ...
  def save_role_json_all(folder_id)
    ...
    #cnn.spec.config[:database] = "#{default_database_name}_#{sprintf('%06d', 1)}"
