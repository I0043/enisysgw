...
class System::User < ActiveRecord::Base
  ...
  has_many :groups, -> {order('system_users_groups.job_order, system_groups.sort_no')}, :through => :user_groups,
    :source => :group
  has_many :user_group_histories, :foreign_key => :user_id,
    :class_name => 'System::UsersGroupHistory'

  has_many :logins, -> {order('id desc')}, :foreign_key => :user_id, :class_name => 'System::LoginLog',
  ...
  scope :extract_editable_user_in_system_users, lambda { |user_id|
    ...
    System::User.includes(:user_groups).where("system_users_groups.group_id in (?) or system_users_groups.group_id is null", editable_group_ids).references(:user_groups)
  }
  ...
  #  boolean
  def system_admin?
    return System::Role.has_auth?(self.id)
  end
  ...
  def doclibrary_admin_role(title_id)
    group_ids = user_group_parent_ids
  ...
  def has_alphanumeric?(str)
    str = str.to_s
    cnt = 0
    cnt += (str =~ /[A-Z]+/).blank? ? 0 : 1
    cnt += (str =~ /[a-z]+/).blank? ? 0 : 1
    cnt += (str =~ /[0-9]+/).blank? ? 0 : 1
    cnt += (str =~ /\A(?=.*?[!-\/:-@\[-`{-~])[!-~]+\z/i).blank? ? 0 : 1
    return cnt >= 3
  end
  ...
  def users_show_group_name(error = Gw.user_groups_error)
    group = self.groups.collect{|x| %Q(#{x.name})}.join(' ')
    if group.blank?
      return error
    else
      return group
  ...
  def self.get_user_select(g_id=nil,all=nil, options = {})
    ...
    order = "code"
    users_select = System::User.where(conditions).select("id,code,name").order(order).joins('left join system_users_groups on system_users.id = system_users_groups.user_id')
  ...
  def self.search(params)
    ...
    params.each do |n, v|
      ...
      when 's_keyword'
        or_cond  = []
        *columns = :code , :name , :name_en , :email
  ...
  # === ユーザーの所属グループコードメソッド
  #  本メソッドは、ユーザーの所属グループコードを取得するメソッドである（親グループを含む）。
  # ==== 引数
  #  なし
  # ==== 戻り値
  #  ユーザーの所属グループコード配列
  def user_group_parent_codes
    group_codes = []
    self.enable_user_groups.each do |user_group|
      group_codes += user_group.group.parent_tree.map(&:code)
    end
    return group_codes.uniq
  end

  # === スケジュール権限判定メソッド
  # スケジュール権限がログインユーザーにあるか判定するメソッド
  def schedule_auth?
