...
class Gw::PropAdminSetting < Gw::Database
  ...
  has_many :prop_admin_setting_roles, -> {order("gw_prop_admin_setting_roles.id")}, :foreign_key => :prop_setting_id, :class_name => 'Gw::PropAdminSettingRole'
  ...
  def admin(pattern = :show, parent_groups = Gw::PropAdminSetting.get_parent_groups)
    admin = Array.new
    groups = System::GroupHistory.where("id in (?)", self.admin_gids).order("level_no,  sort_no , code, start_at DESC, end_at IS Null ,end_at DESC")
    parent_groups.each do |parent_group|
      groups.each do |group|
        ...
        if !g.blank?
          ...
          admin << ["<span class=\"required\">" + I18n.t("rumi.schedule.public_group.delete_group") + " gid=#{group.id}</span>"] if pattern == :show
        end
      end
    end
    return admin.uniq
