...
class System::RoleNamePriv < ActiveRecord::Base
  ...
  validates_uniqueness_of :role_id, scope: [:priv_id], message: I18n.t("rumi.error.role_name_priv.registered")

  def self.get_priv_items(role_id)
    cond  = ["role_id = ?", role_id]
    items = self.where(cond).joins(:priv).order('system_priv_names.sort_no')
    return items
  end
