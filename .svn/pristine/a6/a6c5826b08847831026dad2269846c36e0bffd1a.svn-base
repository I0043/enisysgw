...
module Gw::Model::UserProperty
  def self.get(property_name, options={})
    ...
    if !u.blank? || options[:class_id] == 3
      hx = Gw::UserProperty.where(cond).first
  ...
  def self.save(property_name, h_save, options={})
    ...
    when 'feeds'
      ...
      feeds = item
      if feeds.nil?
        errors.push ['feeds', I18n.t("rumi.message.data_error")]
      else
        feeds.each do |feed|
          'uri:title:max'.split(':').each do |x|
            fld = trans.rassoc(x)[0] rescue x
            errors.push [x, "#{fld}" + I18n.t("rumi.error.input")] if feed[x].blank?
          end
        end
      end
    when 'schedules'
     if options[:class_id] == 1
      trans.each do |x|
        fld = x[1]
        fld_disp = x[0]
        errors.push [fld, "#{fld_disp}" + I18n.t("rumi.error.input")] if item[fld].blank?
      end
      ...
      %w(week_view_piecehead_format week_view_dayhead_format month_view_piecehead_format month_view_dayhead_format).each do |x|
        ...
        if !item[x].blank?
          ...
          errors.push [x, "#{fld}" + I18n.t("rumi.error.abnormality")] if err.nil?
        end
      end
        ...
        if !x.nil?
          if Gw.int?(x)
            if !((0..6).include?(x.to_i))
              errors.push [fld, "#{fld_disp}" + I18n.t("rumi.error.number0_of_6")]
            end
          else
            errors.push [fld, "#{fld_disp}" + I18n.t("rumi.error.number0_of_6")]
          end
      ...
      %w(header_each).each do |fld|
        ...
        if !x.blank? && (!Gw.int?(x) || x.to_i < 0 )
          errors.push [fld, "#{fld_disp}" + I18n.t("rumi.error.not_zero")]
        end
      end
     end
    when 'ssos'
      ...
      itm1 = item[trans[0][1]]
      itm2 = item[trans[1][1]]
      errors.push [trans[0][1], "#{trans[0][0]}" + I18n.t("rumi.error.input")] if itm1.blank? && !itm2.blank?
      errors.push [trans[1][1], "#{trans[1][0]}" + I18n.t("rumi.error.input")] if !itm1.blank? && itm2.blank?
    when 'mobiles'
      ...
      if item['ktrans'].to_s=='2'
        ...
        %w(kmail).each do |fld|
          ...
          when 0
            errors.push [fld, "#{fld_disp}" + I18n.t("rumi.error.not_email")]
            ret = 1
          when 2
            ret = 2
          else
    ...
    when 'blogparts'
      ...
      raise TypeError, I18n.t("rumi.error.undefined_type") + "(#{key})"
    end
    ...
    if options[:class_id].blank? || options[:class_id] == 1
      uid = Core.user.id
      hx = Gw::UserProperty.where(:uid=>uid, :name=>property_name, :class_id => 1).first
      hx = Gw::UserProperty.new({:class_id=>1, :uid=>uid.to_s, :name=>property_name, :type_name=>'json', :options => ''}) if hx.nil?
    elsif options[:class_id] == 2

    elsif options[:class_id] == 3
      uid = Core.user.id
      hx = Gw::UserProperty.where(:name=>property_name, :class_id => 3).first
