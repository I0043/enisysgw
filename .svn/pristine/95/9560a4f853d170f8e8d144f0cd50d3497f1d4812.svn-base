...
protected
  ...
  def _create(item, options = {})
    ...
    respond_to do |format|
      if item.creatable? && item.save(:validate => (!ignore_validate))
        ...
        flash[:notice] = options[:notice] || I18n.t("rumi.message.notice.create")
        ...
        if @title.is_a?(Gwcircular::Control) && item.is_a?(Gwcircular::Doc)
          ...
          if item.doc_type == 0 && item.state == 'public'
            if item.created_at == item.latest_updated_at
              # 回覧作成の場合、配信者のユーザーへ新着情報(作成時)を通知
              item.build_created_remind
            else
              # 配信者のユーザーへ新着情報(更新時)を通知
              item.build_updated_remind
            end
          end

          if item.doc_type == 1
            if @before_state == 'unread' && item.state == 'already'
              # 新着情報を既読に変更
              item.parent_doc.seen_remind(Core.user.id)
            end

            #  配信者のユーザーへ新着情報(コメント追加)を通知
            item.parent_doc.build_reply_remind
          end
        end

        flash[:notice] = options[:notice] || I18n.t("rumi.message.notice.update")
        location = options[:success_redirect_uri] ||= url_for(:action => :index)
        format.html { redirect_to location }
        format.xml  { head :ok }
      else
        if options[:request_path].present?
          format.html { render :action => failured_action || :edit, layout: "admin/template/forward_form" }
          format.xml  { render :xml => item.errors, :status => :unprocessable_entity }
        else
  ...
  def _update_plus_location(item, s_location, options = {})
    respond_to do |format|
      if item.editable? && item.save
        ...
        if @title.is_a?(Gwbbs::Control) && item.is_a?(Gwbbs::Doc)
          ...
          if item.public_status?
            ...
            if has_seen_or_unseen_reminders
              # 該当記事の更新用新着情報を作成する
              unless item.build_updated_remind
                options[:notice] = '新着情報配信対象者が存在しません。掲示板の設定を見直してください。'
              end
            else
              # 該当記事の公開用新着情報を作成する
              unless item.build_created_remind
                options[:notice] = '新着情報配信対象者が存在しません。掲示板の設定を見直してください。'
              end
            end
          end
        ...
        if options[:request_path].present?
          format.html { render :action => :edit, layout: "admin/template/forward_form" }
          format.xml  { render :xml => item.errors, :status => :unprocessable_entity }
        else
  ...
  def destroy_recognizers(recog_item)
    ...
    sql.where
    recog_item.where(sql.where).destroy_all
  end
  ...
  def _destroy(item, options = {})
    respond_to do |format|
      if item.deletable? && item.destroy
        ...
        flash[:notice] = options[:notice] || I18n.t("rumi.message.notice.delete")
        location = options[:success_redirect_uri] ||= url_for(:action => :index)
        format.html { redirect_to location}
        format.xml  { head :ok }
      else
        flash[:notice] = I18n.t("rumi.message.notice.delete_fail")
  ...
  def error_gwbbs_no_title
    error I18n.t('rumi.gwbbs.message.bbs_fail')
  end
end
