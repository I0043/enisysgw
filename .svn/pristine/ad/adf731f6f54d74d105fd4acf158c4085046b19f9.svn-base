...
class System::GroupHistory < ActiveRecord::Base
  ...
  has_and_belongs_to_many :users, -> {order('job_order,system_users.name_en')}, :class_name => 'System::User',
    :join_table => 'system_users_group_histories'
  ...
  validates_each :end_at do |record, attr, value|
    record.errors.add attr, I18n.t("rumi.error.group.after_start_at") if value.blank? ? false : (value <= record.start_at)
  end
  ...
  def display_name_with_level_no
    return ["+", "--" * (self.level_no.to_i - 1), " ", "(#{code}) #{name}"].join
  end
  ...
  def self.select_dd_group(day=nil,level=nil,parent_id=nil,all=nil)
    ...
    if parent_id ==nil
      ...
      groups = System::GroupHistory.where(g_cond)
      groups.each do |g|
