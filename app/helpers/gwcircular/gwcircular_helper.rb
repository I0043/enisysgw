...
module Gwcircular::GwcircularHelper
  ...
  def status_name(item)
    str = ''
    if item.doc_type == 0
      str = t("rumi.state.draft") if item.state == 'draft'
      str = t("rumi.state.public") if item.state == 'public'
      str = t("rumi.state.public_end") if item.expiry_date < Time.now unless item.expiry_date.blank? if item.state == 'public'
    end
    if item.doc_type == 1
      str = t("rumi.state.preparation") if item.state == 'preparation'
      str = t("rumi.state.still") if item.state == 'draft'
      str = '<span class="required">' + t("rumi.state.unread") + '</span>' if item.state == 'unread'
      str = '<span class="notice">' + t("rumi.state.already") + '</span>' if item.state == 'already'
      str = t("rumi.state.public_limit") if item.expiry_date < Time.now unless item.expiry_date.blank? if item.state == 'public'
    end
  ...
  def file_show_path(item, params)
    case params[:cond]
    when 'unread'
      condition = "state='unread' AND doc_type=1 AND target_user_code='#{Core.user.code}' AND parent_id='#{item.id}'"
      item = Gwcircular::Doc.where(condition).first
    when 'already'
      condition = "state='already' AND doc_type=1 AND target_user_code='#{Core.user.code}' AND parent_id='#{item.id}'"
      item = Gwcircular::Doc.where(condition).first
    end
