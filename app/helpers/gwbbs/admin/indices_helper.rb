...
module Gwbbs::Admin::IndicesHelper
  ...
  def ret_str_gwbbs_search_title_lbl
    str = ''
    # 検索結果の文言、デザイン変更
    unless params[:kwd].blank? and params[:creater].blank? and
      ...
      str += '<h4 class="docSearchTitle rumi-search-title">検索結果</h4>' 
      str += '</div>'
    end
    return str
  end
  ...
  def gwbbs_form_indices_date
    ...
    i = 0
    brk_key = nil
    ret = ret_str_gwbbs_search_title_lbl
    ret += '<table class="index">'
    ret += '<tr class="headIndex">'
    ret += '<th class="item">' + "</th>" if is_read_check?
    ...
    for item in @items
      unless brk_key == item.latest_updated_at.strftime('%Y-%m-%d').to_s
        ...
        ret += '<th colspan="7" class="docTitle">' + item.latest_updated_at.strftime('%Y-%m-%d').to_s  + "</th>" if @title.importance.to_s == '1' && is_read_check?
        ret += '<th colspan="6" class="docTitle">' + item.latest_updated_at.strftime('%Y-%m-%d').to_s  + "</th>" if (@title.importance.to_s != '1' && is_read_check?) || (@title.importance.to_s == '1' && !is_read_check?)
        ret += '<th colspan="5" class="docTitle">' + item.latest_updated_at.strftime('%Y-%m-%d').to_s  + "</th>" if (@title.importance.to_s != '1' && !is_read_check?)
        ret += "</tr>"
      end

      new_mark_str = add_new_mark(item, params[:state])
=begin
      if @title.id == 1
...
=end
      ...
      ret += tr.to_s
      ret += %Q(<td class="checkRead"><input type="checkbox" id="ids_#{item.id}" name="ids[#{item.id}]" value="1" /></td>) if is_read_check?
      ret += '<td class="bbsImportant" style="text-align: center;">' + retstr_important_span(item.importance) + "</td>" if @title.importance.to_s == '1'

      # 未読(下書一覧、公開前一覧、期限切れ表示を除く)の場合、強調表示
      ret += '<td class="docTitle">' + decorate_title_link(item, params[:state], link_to(hbr(item.title), item.show_path  + gwbbs_params_set)) + new_mark_str + "</td>"
  ...
  def gwbbs_form_indices_group
    ...
    for item in @items
      unless brk_key == item.section_code.to_s
        i = 0
        ret += '<tr class="subIndex">'
        ret += '<th colspan="7" class="docTitle">' + item.section_name.to_s + "</th>" if @title.importance.to_s == '1' && is_read_check?
        ret += '<th colspan="6" class="docTitle">' + item.section_name.to_s + "</th>" if (@title.importance.to_s != '1' && is_read_check?) || (@title.importance.to_s == '1' && !is_read_check?)
        ret += '<th colspan="5" class="docTitle">' + item.section_name.to_s + "</th>" if (@title.importance.to_s != '1' && !is_read_check?)
        ret += "</tr>"
      end
  ...
  def gwbbs_form_indices_category
    ...
    for item in @items
      ...
      unless brk_key == item.category1_id.to_s
        i = 0
        ret += '<tr class="subIndex">'
        ret += '<th colspan="7" class="docTitle">' + category_name + "</th>" if @title.importance.to_s == '1' && is_read_check?
        ret += '<th colspan="6" class="docTitle">' + category_name + "</th>" if (@title.importance.to_s != '1' && is_read_check?) || (@title.importance.to_s == '1' && !is_read_check?)
        ret += '<th colspan="5" class="docTitle">' + category_name + "</th>" if (@title.importance.to_s != '1' && !is_read_check?)
        ret += "</tr>"
      end
...
=end
  ...
  # === 未読記事か判断し、強調表示するメソッド
  #  公開記事一覧かつ、標準表示、月別表示、所属表示の場合のみ未読記事を強調表示する
  # ==== 引数
  #  * item: Gwbbs::Doc
  #  * state_str: タブ種別
  #  * title_link: リンク
  # ==== 戻り値
  #  HTMLの文字列
  def decorate_title_link(item, state, title_link)
    # 下書き一覧、公開前一覧、期限切れ一覧を除く
    # 強調表示機能はなくなったため、すべて普通のタイトルを返すよう変更
    #if !["DRAFT", "NEVER", "VOID"].include?(state) && item.unseen?(Core.user.id)
    #  return "<b>" + title_link + "</b>"
    #else
      return title_link
    #end
  end
  
  # === 未読記事か判断し、Newアイコンを表示するメソッド
  #  公開記事一覧かつ、標準表示、月別表示、所属表示の場合のみ未読記事にNewアイコンを表示する
  # ==== 引数
  #  * item: Gwbbs::Doc
  #  * state_str: タブ種別
  #  * title_link: リンク
  # ==== 戻り値
  #  HTMLの文字列
  def add_new_mark(item, state)
    # 下書き一覧、公開前一覧、期限切れ一覧を除く
    if !["DRAFT", "NEVER", "VOID"].include?(state) && item.unseen?(Core.user.id)
      return %Q(<span class="new">new</span>)
    else
      return ''
    end
  end
  ...
  def is_read_check?
    return true if !["DRAFT", "NEVER", "VOID"].include?(params[:state])
    return false
  end
