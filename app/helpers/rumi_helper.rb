...
module RumiHelper
  ...
  # === 新着情報（コメント時）に表示する概要文が40文字（デフォルト）以上の時、文字を切り捨てるメソッド
  #  概要文(上限40文字) + " のコメントが○件追加されました。"
  # ==== 引数
  #  * category: 機能を表すsymbol
  #  * action: 概要文に付加する操作を表すsymbol
  #  * title: 概要文
  ...
  # ==== 戻り値
  #  概要文 + 概要文に付加する操作
  #    例: 回覧板題名 のコメントが1件追加されました。
  def truncate_reply_remind_title(category, action, title, reply_cnt, word_count = 40)
    sep = title.length > word_count ? "... " : " "
    comment = t(["rumi.reminder.feature", category, "action", action, "comment"].join("."))
    add = t(["rumi.reminder.feature", category, "action", action, "add"].join("."))
    count = t("rumi.reminder.count")
    return truncate(title, length: word_count, omission: "") + sep + comment + "#{reply_cnt}" + count + add
  end
  ...
  def span_notification_count(link_options, user_code, password)
    if mail_feature_url?(link_options[:url])
      return %Q(<p id="mailcountspan"><span id="notificationMailCount" class="noRead">---</span></p>)
    end
  ...
  def link_to_show_profile(display_name, user_code)
    user = System::User.where(code: user_code).first
    return link_to(display_name, "/system/users/#{user.id}/show_profile")
  end
  ...
  def get_header_menus
    if Core.user
      @header_menus_items = Rumi::PieceApi.header_menus(URI.parse(root_url), Core.user.code, Core.user.password)
    else
      @header_menus_items = ""
    end
  end

  def get_smart_header_menus
  ...
  def get_reminder_image(title)
    image_url = ""

    title_name = title.is_a?(Array) ? title[0] : title
    link_piece = Gw::EditLinkPiece.where(level_no: 3, name: title_name).first
    if link_piece.present?
      image_url = link_piece.icon_path
    end
    return image_url
  end
  ...
  # === スマホ版ログアウト時に遷移するURLを生成する
  # ===
  # ==== 引数
  #  * なし
  # ==== 戻り値
  #  ログアウト時に遷移するURL
  def make_smart_logout_url
    url = Enisys::Config.application["webmail.root_url"]

    if url.blank?
      return '/_admin/smart_login'
    else
      begin
        logout_url = URI.join(url, "/_admin/logout").to_s
      rescue
