...
class Gw::Admin::Piece::ReminderController < ApplicationController
  ...
  def index
    ...
    tmp_user = System::User.where(code: params[:code]).first
  ...
  def all_seen_remind
    ...
    if params[:checkd_reminders_circular] and params[:checkd_reminders_circular].size > 0
      ...
      params[:checkd_reminders_circular].each do |key, value|
        ...
        if reminder.present?
          #コメントの新着情報の場合、該当記事のコメント新着を全て既読にする
          if reminder.action == "reply"
            comment_reminders = Gw::Reminder.where(category: reminder.category, title_id: reminder.title_id)
                                            .where(item_id: reminder.item_id, action: "reply")
            comment_reminders.update_all(seen_at: Time.now())
          end
          reminder.seen
          # 対象回覧板記事の更新
