...
#    @st_date = kd =~ /[0-9]{8}/ ? Date.strptime(kd, '%Y%m%d') : Date.today
    if params['s_year'].present?
      @st_date = Date.today
    else
      if Date.valid_civil?(params[:period_from][:year].to_i, params[:period_from][:month].to_i, params[:period_from][:day].to_i)
       @st_date =  Date.civil(params[:period_from][:year].to_i, params[:period_from][:month].to_i, params[:period_from][:day].to_i)
      else  
        @st_date = Date.today
      end
    end    
    ...
    if params['s_year'].present?
      @ed_date = Date.today
#      @ed_date = Date.today.end_of_year
    else
      if Date.valid_civil?(params[:period_to][:year].to_i, params[:period_to][:month].to_i, params[:period_to][:day].to_i)
        @ed_date =  Date.civil(params[:period_to][:year].to_i, params[:period_to][:month].to_i, params[:period_to][:day].to_i)
      else
        @ed_date = Date.today.end_of_year
      end 
    end
...
# 長島追記
    @s_category = nz(params[:s_category], [])
    @s_user = nz(params[:s_user], 0).to_i 
    @s_keyword = nz(params[:s_keyword], 0)
...
=end
    ...
    cond = "(#{@db_name}.st_at <= '#{@period_to} 23:59:59' and '#{@period_from} 00:00:00' <= #{@db_name}.ed_at)" 

    unless @s_category.blank?
      cond += " and #{@db_name}.title_category_id in (#{@s_category.join(",")})"
    end

    if @s_keyword != 0
      cond += " and (#{@db_name}.title like '%#{@s_keyword}%' or #{@db_name}.place like '%#{@s_keyword}%' or #{@db_name}.memo like '%#{@s_keyword}%')"
    end
    ...
    if @s_user == 0    
      @items = item.where(cond).order("#{@db_name}.st_at")
    else
      if params[:uid].present?
        @uid_equal = (params[:uid].to_i == @uid)
        @list_user = System::User.find_by_id(params[:uid])
        @list_group = @list_user.groups[0]
        cond += " and gw_schedule_users.uid = #{params[:uid]}"
      else
        cond += " and gw_schedule_users.uid = #{@uid}"
        @uid_equal = true
      end
