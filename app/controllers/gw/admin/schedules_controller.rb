...
class Gw::Admin::SchedulesController < Gw::Controller::Admin::Base
  ...
  before_action :set_request_from, only: [:index, :show, :show_month]

  def initialize_scaffold
    return redirect_to(request.env['PATH_INFO']) if params[:reset]
    Page.title = t("rumi.schedule.name")
  ...
  def _schedule_data
    if @gw_admin || params[:cgid].blank? ||
        ( params[:cgid].present? && System::CustomGroupRole.editable?( params[:cgid], Core.user_group.id, Core.user.id ) )
      @edit = true
    else
      @edit = false
    end
  ...
  def show_one
    ...
    if @item.schedule_repeat_id.present?
      repeat_items = Gw::Schedule.where(schedule_repeat_id: @item.repeat.id)
  ...
  def create
    ...
    @item = Gw::Schedule.new(item_params)
  ...
  def delete_schedule
    ...
    if othlinkflg
      redirect_url = "/gw/schedules/#{@item.id}/show_one"
    else
      redirect_url = "/gw/schedules/#{@item.id}/show_one"
    end
  ...
  def destroy
    ...
    if othlinkflg
      location = "/gw/schedules/#{@item.id}/show_one"
    else
      location = "/gw/schedules/#{@item.id}/show_one"
    end
  ...
  def setting_gw_link
    init_params
    @item = Gw::SystemProperty.where(name: 'gw_link').first
  end
  ...
  def edit_ind(key)
    init_params
    options = {}
    raise ArgumentError, t("rumi.message.incorrect_call") if %w(schedules ssos mobiles).index(key).nil?
  ...
  def edit_ind_core(key, options={})
    ...
    if key=='mobiles'
      ...
      when 2
        flash[:notice] = t("rumi.message.notice.update") + t("rumi.schedule.message.address_format")
        redirect_to "/gw/memo_settings"
      else
        respond_to do |format|
          format.html {
      ...
      if ret == true
        if key=='ssos'
          ...
          redirect_to gw_config_settings_path
        end
      else
        respond_to do |format|
          format.html {
  ...
  def print_index
    index
    render partial: "show_week", layout: false, locals: {place: "print"}
  end

  # === 印刷プレビュー表示メソッド
  def print_show_month
    show_month
    render partial: "show_month", layout: false, locals: {place: "print"}
  end
  ...
  def set_mobile_params(params_i)
    ...
    if params_o[:item][:public_groups].blank?
      ...
      params_o[:item][:public_groups].each do |g|
        if g[1].to_i != 0
          group_name = System::Group.find_by(id: g[1])
  ...
  def reject_no_necessary_params(params_i)
    ...
    when "1"
      ...
      params_o[:item][:st_at] = params_o[:item][:st_at].to_time.strftime("%Y-%m-%d %H:%M")
      params_o[:item][:ed_at] = params_o[:item][:ed_at].to_time.strftime("%Y-%m-%d %H:%M")
    when "2"
      ...
      raise Gw::ApplicationError, t("rumi.schedule.message.valid") + "(repeat_mode=#{params_o[:init][:repeat_mode]})"
    end
  ...
  def item_params
    params.fetch(:item, {})
          .permit(:st_at, :ed_at, :creator_uid, :creator_uname, :creator_ucode, :creator_gid, :created_at,
                  :allday, :title_category_id, :title, :place, :memo, :owner_uid,
                  :inquire_to, :is_public, :schedule_props)
  end
