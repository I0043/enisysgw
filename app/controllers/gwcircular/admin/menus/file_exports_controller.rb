...
class Gwcircular::Admin::Menus::FileExportsController < Gw::Controller::Admin::Base
  ...
  def export_file
    ...
    @is_readable = false unless @item.target_user_code == Core.user.code unless @gw_admin
    return authentication_error(403) unless @is_readable
    target_folder = Rails.root.join("tmp/gwcircular/#{sprintf('%06d',@item.id)}/")
    ...
    docs = Gwcircular::Doc.where(title_id: @title.id, doc_type: 1, parent_id: @item.id)
                         .where("state != 'preparation'")
    
    i = 0
    for doci in docs
      files = Gwcircular::File.where(title_id: 1, parent_id: doci.id)
                             .order('id')
