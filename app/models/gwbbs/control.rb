...
class Gwbbs::Control < Gw::Database
  ...
  has_many :docs, :foreign_key => :title_id, :class_name => 'Gwbbs::Doc',:dependent => :destroy
  has_many :itemdeletes, :foreign_key => :title_id, :class_name => 'Gwbbs::Itemdelete',:dependent => :destroy
  ...
  def save_admingrps
    unless self.admingrps_json.blank?
      Gwbbs::Adm.where("title_id=#{self.id}").destroy_all
  ...
  def save_editors
    unless self.editors_json.blank?
      Gwbbs::Role.where("title_id=#{self.id} and role_code = 'w'").destroy_all
  ...
  def save_readers
    unless self.readers_json.blank?
      Gwbbs::Role.where("title_id=#{self.id} and role_code = 'r'").destroy_all
  ...
  def save_readers_add
    unless self.editors_json.blank?
      item = Gwbbs::Role.where("title_id=#{self.id} and role_code = 'r' and group_code = '0'")
      if item.length == 0
        groups = JsonParser.new.parse(self.editors_json)
        groups.each do |group|
          unless group[1].blank?
            item_grp = Gwbbs::Role.where("title_id=#{self.id} and role_code = 'r' and group_id = #{group[1]}")
  ...
  def validate_params
    errors.add :default_published, I18n.t('rumi.gwbbs.message.validate_num') if self.default_published.blank?
    errors.add :default_published, I18n.t('rumi.gwbbs.message.validate_num') if self.default_published == 0
    errors.add :upload_graphic_file_size_capacity, I18n.t('rumi.gwbbs.message.validate_num') if self.upload_graphic_file_size_capacity.blank?
    errors.add :upload_graphic_file_size_capacity, I18n.t('rumi.gwbbs.message.validate_num') if self.upload_graphic_file_size_capacity == 0
    errors.add :upload_document_file_size_capacity, I18n.t('rumi.gwbbs.message.validate_num') if self.upload_document_file_size_capacity.blank?
    errors.add :upload_document_file_size_capacity, I18n.t('rumi.gwbbs.message.validate_num') if self.upload_document_file_size_capacity == 0
    errors.add :upload_graphic_file_size_max, I18n.t('rumi.gwbbs.message.validate_num') if self.upload_graphic_file_size_max.blank?
    errors.add :upload_graphic_file_size_max, I18n.t('rumi.gwbbs.message.validate_num') if self.upload_graphic_file_size_max == 0
    errors.add :upload_document_file_size_max, I18n.t('rumi.gwbbs.message.validate_num') if self.upload_document_file_size_max.blank?
    errors.add :upload_document_file_size_max, I18n.t('rumi.gwbbs.message.validate_num') if self.upload_document_file_size_max == 0
    errors.add :doc_body_size_capacity, I18n.t('rumi.gwbbs.message.validate_num') if self.doc_body_size_capacity.blank?
    errors.add :doc_body_size_capacity, I18n.t('rumi.gwbbs.message.validate_num') if self.doc_body_size_capacity == 0

    error_users = validate_users(self.adms_json) unless self.adms_json.blank?
    unless error_users.blank?
      error_user = error_users.join(", ")
      errors.add(:adms_json, I18n.t('rumi.gwbbs.message.invalid_user', error_user: error_user))
    end

    error_users = validate_users(self.sueditors_json) unless self.sueditors_json.blank?
    unless error_users.blank?
      error_user = error_users.join(", ")
      errors.add(:sueditors_json, I18n.t('rumi.gwbbs.message.invalid_user', error_user: error_user))
    end

    error_users = validate_users(self.sureaders_json) unless self.sureaders_json.blank?
    unless error_users.blank?
      error_user = error_users.join(", ")
      errors.add(:sureaders_json, I18n.t('rumi.gwbbs.message.invalid_user', error_user: error_user))
    end
  end
  ...
  class << self
    ...
    def remind(user_id, sort_key, order)
      gwbbs_records = Gw::Reminder.extract_bbs(user_id, sort_key, order)
      total_count = gwbbs_records.count
      reply_cnt = 0
      relation = Array.new
      reply_count = Array.new
      gwbbs_record_urls = Array.new
      gwbbs_records.each do |record|
        break if relation.count >= Enisys.config.application['gw.reminder_view_size'].to_i
        if record.action == 'reply'
          record_exist = gwbbs_record_urls.find{|x| x == record.url}
          next if record_exist.present?

          record_cnt = gwbbs_records.where(url: record.url, action: 'reply').count
          relation << record
          reply_count << record_cnt
          total_count = total_count - record_cnt + 1
          gwbbs_record_urls << record.url
        else
          relation << record
          reply_count << 0
          reply_cnt = 0
    ...
    # === 通知件数取得メソッド
    #  ログインユーザーが閲覧権限を持つ掲示板において公開された未読の記事件数及び
    #  ログインユーザーの所属が記事管理課に指定されている未読のコメントが存在する記事件数を取得する
    # ==== 引数
    #  * user_id: ユーザーID
    # ==== 戻り値
    #  通知件数(整数)
    def notification(user_id)
      record_cnt = 0
      gwbbs_record_urls = Array.new
      gwbbs_records = Gw::Reminder.extract_bbs(user_id, nil, nil)
      gwbbs_records.each do |record|
        if record.action == 'reply'
          record_exist = gwbbs_record_urls.find{|x| x == record.url}
          next if record_exist.present?
    ...
    def notification_counts(user_id)
      ...
      Gw::Reminder.extract_bbs(user_id, nil, nil).
          select("title_id, count(distinct item_id) AS item_count").
