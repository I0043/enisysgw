...
class System::UsersGroupsCsvdata < ActiveRecord::Base
  ...
  has_many :groups, -> {where(data_type: "group")}, :foreign_key => :parent_id, :class_name => 'System::UsersGroupsCsvdata'
  has_many :users, -> {where(data_type: "user")}, :foreign_key => :parent_id, :class_name => 'System::UsersGroupsCsvdata'
  ...
  validates :code, uniqueness: { scope: [:parent_id, :data_type] }
  # 状態が無効の場合は入力必須
  ...
  def self.import_csv(csv_string)
    ...
    begin
      ...
      ActiveRecord::Base.transaction do
        ...
        raise I18n.t("rumi.csv.message.nadequacy") if has_invalid
      end
  ...
  def self.parse_csv(csv_hash_row)
    ...
    if parsed_csv_data.data_type_user?
      ...
      # DBに同一codeのユーザーが存在し、CSV項目のパスワードが空の場合、DB情報で上書きする
      db_user = System::User.where(code: parsed_csv_data.code).first
      if db_user.present? && parsed_csv_data.password.blank?
        parsed_csv_data.password = db_user.password
      end
  ...
  def self.data_type_show(data_type)
    return I18n.t("rumi.csv.data_type.#{data_type}")
  end
