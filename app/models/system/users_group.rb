...
class System::UsersGroup < ActiveRecord::Base
  ...
  validates_uniqueness_of :group_id, scope: :user_id, message: I18n.t("rumi.error.registered")
  ...
  validates :group_id, inclusion: { in: Proc.new{ |record| Core.user.editable_groups_in_system_users.without_disable.map(&:id) },
  ...
  validates_each :end_at do |record, attr, value|
    user = System::User.find_by(id: record.user_id)
    if value.present?
      record.errors.add attr, I18n.t("rumi.error.users_group.enable_end_at_blank") if user.present? && user.state == "enabled"
      record.errors.add attr, I18n.t("rumi.error.users_group.after_start_at") if record.start_at.present? && Time.local(value.year, value.month, value.day, 0, 0, 0) < Time.local(record.start_at.year, record.start_at.month, record.start_at.day, 0, 0, 0)
      record.errors.add attr, I18n.t("rumi.error.users_group.before_today") if Time.local(value.year, value.month, value.day, 0, 0, 0) > Time.local(Time.now.year, Time.now.month, Time.now.day, 0, 0, 0)
    end
  end
  ...
  default_scope -> { order("system_users_groups.job_order is null",
  ...
  #  なし
  def user_id_valid
    is_editable_user = Core.user.editable_user_in_system_users?(user_id)
  ...
  def set_columns
    if self.user_id.to_i==0
        self.user_code  = I18n.t("rumi.users_group.message.unregistered_user")
    else
      if self.user.blank?
        self.user_code  = I18n.t("rumi.users_group.message.unregistered_user")
      else
    ...
    if self.group_id.to_i==0
        self.group_code  = I18n.t("rumi.users_group.message.unregistered_group")
    else
      if self.group.blank?
        self.group_code  = I18n.t("rumi.users_group.message.unregistered_group")
      else
  ...
  def to_csv
    ...
    csv = [
      ...
      user.name, user.name_en, "", user.email, user.sort_no, user.official_position, user.assigned_job,
