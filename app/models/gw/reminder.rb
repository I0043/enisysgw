...
class Gw::Reminder < Gw::Database
  ...
  # === 機能：掲示板においてコメント未読のものを抽出するためのスコープ
  #
  # ==== 引数
  #  * なし
  # ==== 戻り値
  #  抽出結果(ActiveRecord::Relation)
  scope :extract_reply, lambda {
    # 操作名が'request'のものを抽出
    where(action: 'reply')
  }
  
  # === 機能：掲示板においてコメント未読以外のものを抽出するためのスコープ
  #
  # ==== 引数
  #  * なし
  # ==== 戻り値
  #  抽出結果(ActiveRecord::Relation)
  scope :extract_not_reply, lambda {
    # 操作名が'request'のものを抽出
    where.not(action: 'reply')
  }
  ...
  class << self
    ...
    def to_rumi_format(relation)
      ...
      return {
        ...
        factors: relation.limit(Enisys.config.application['gw.reminder_view_size'].to_i).to_a
      }
    end
