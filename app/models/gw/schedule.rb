...
class Gw::Schedule < Gw::Database
  ...
  validates_each :st_at do |record, attr, value|
    ...
    record.errors.add attr, I18n.t("rumi.error.schedule.context") if d_st > d_ed
  end
  ...
  has_many :normal_reminders, -> {where(category: "schedule")}, foreign_key: :item_id, class_name: "Gw::Reminder", dependent: :destroy
  has_many :prop_reminders, -> {where(category: "schedule_prop")}, foreign_key: :item_id, class_name: "Gw::Reminder", dependent: :destroy
  has_many :reminders, -> {where("category like ? and sub_category is null", "schedule%")}, foreign_key: :item_id, class_name: "Gw::Reminder"
    ...
    proc_core = lambda{
      ...
      if mode == :update
        ...
        return false if !item.update_attributes(di
          .permit(:st_at, :ed_at, :creator_uid, :creator_uname, :creator_ucode, :creator_gid, :created_at, :allday, :title_category_id,
                  :title, :place, :memo, :owner_uid, :inquire_to, :is_public, :to_go, :participant_nums_inner, :participant_nums_outer,
                  :check_30_over, :admin_memo, :guide_state, :guide_place_id, :guide_place, :guide_ed_at, :updater_uid,
                  :updater_ucode, :updater_uname, :updater_gid, :updater_gcode, :updater_gname, :creator_gcode,
                  :creator_gname, :owner_ucode, :owner_uname, :owner_gid, :owner_gcode, :owner_gname, :schedule_repeat_id))
        ...
        return false if !item.update_attributes(ActionController::Parameters.new(di)
          .permit(:st_at, :ed_at, :creator_uid, :creator_uname, :creator_ucode, :creator_gid, :allday, :title_category_id,
                  :title, :place, :memo, :owner_uid, :inquire_to, :is_public, :to_go, :participant_nums_inner, :participant_nums_outer,
                  :check_30_over, :admin_memo, :guide_state, :guide_place_id, :guide_place, :guide_ed_at, :updater_uid,
                  :updater_ucode, :updater_uname, :updater_gid, :updater_gcode, :updater_gname, :creator_gcode,
                  :creator_gname, :owner_ucode, :owner_uname, :owner_gid, :owner_gcode, :owner_gname, :schedule_repeat_id))
  ...
  def self.separate_repeat_params(params)
    item_main = ActionController::Parameters.new
    item_repeat = ActionController::Parameters.new
  ...
  def  get_repeat_items
    ...
    return self.where("schedule_repeat_id='#{self.schedule_repeat_id}'").order("st_at, id")
  end
  ...
  def repeat_end_str
    ...
    return " ～#{item.ed_at.day.to_s}" + I18n.t("rumi.strftime.day")
  end
  ...
  def get_propnames
    schedule_props = Gw::ScheduleProp.where("schedule_id = ?", self.id)
    _names = Array.new
    names = ""
  ...
  def get_edit_delete_level(auth = {})
    ...
    #施設マスタ権限を持つユーザーかの情報
    props.each do |prop|
      unless Gw::PropOtherRole.is_admin?(prop.prop.id) || Gw::PropOtherRole.is_edit?(prop.prop.id) || @gw_admin
        prop_admin = false
      end
    end
    if prop_admin
...
EOL
    ...
    self.public_roles.each do |public_role|
      if public_role.class_id == 2
        if public_role.uid == 0
          ret << I18n.t("rumi.schedule.public_group.no_limit")
        else
          group = System::GroupHistory.find_by(id: public_role.uid)
          if !group.blank?
            ...
            ret << "<span class=\"required\">" + I18n.t("rumi.schedule.public_group.delete_group") + " gid=#{public_role.uid}</span>"
          end
        end
      end
    end
  ...
  def self.repeat_weekday_select
    items = [[I18n.t("rumi.week.sunday"), 0], [I18n.t("rumi.week.monday"), 1], [I18n.t("rumi.week.tuesday"), 2], [I18n.t("rumi.week.wednesday"), 3], [I18n.t("rumi.week.thursday"), 4], [I18n.t("rumi.week.friday"), 5], [I18n.t("rumi.week.saturday"), 6]]
    return items
  end
  def self.repeat_weekday_show
    is_public_items = [[I18n.t("rumi.schedule.public.open"), 1],[I18n.t("rumi.schedule.public.group"), 2],[I18n.t("rumi.schedule.public.close"), 3]]
  ...
  def self.is_public_show(is_public)
    is_public_items = [[1,I18n.t("rumi.schedule.public.open")],[2,I18n.t("rumi.schedule.public.group")],[3,I18n.t("rumi.schedule.public.close")]]
  ...
  def time_show
    if nz(self.allday, 0) == 0
      ...
      st_at_s = I18n.t("rumi.schedule.schedule_title.time_undecided")
      ed_at_s = I18n.t("rumi.schedule.schedule_title.time_undecided")
    elsif self.allday == 2
      st_at_s = I18n.t("rumi.schedule.schedule_title.time_all")
      ed_at_s = I18n.t("rumi.schedule.schedule_title.time_all")
    end
  ...
  def show_time(date, view = :pc)
    ...
    when 1
      if view == :pc
        return "(#{I18n.t("rumi.schedule.schedule_title.time_undecided")})"
      else
        return I18n.t("rumi.schedule.schedule_title.time_undecided")
      end
    when 2
      if view == :pc
        return "(#{I18n.t("rumi.schedule.schedule_title.time_all")})"
      else
        return I18n.t("rumi.schedule.schedule_title.time_all")
      end
    else
  ...
  def self.find_for_show_schedule(uids, first_day, end_day,user_id)
    ...
    join = "left outer join (select gw_reminders.item_id,user_id,seen_at from gw_reminders" +
          ...
          " and gw_reminders.user_id = #{Core.user.id}) reminder" +
