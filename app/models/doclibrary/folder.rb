# -*- encoding: utf-8 -*-
class Doclibrary::Folder < Gw::Database
  ...
  def tree_level
    if self.level_no > Enisys::Config.application["doclibrary.level_no"].to_i
      tree_level_no = Enisys::Config.application["doclibrary.level_no"].to_s
      errors.add :level_no, I18n.t('rumi.doclibrary.message.tree_level_limit', tree_level_no: tree_level_no)
    end
  end
  ...
  def self.without_admin_auth(title_id)
    ...
    return [] if Core.user.admin_in_doclibrarys?(title_id)

    without_admin_ids = []
    root_folder = Doclibrary::Folder.where(title_id: title_id, level_no: 1).first
    without_admin_ids =
  ...
  def delete_acl_records
    Doclibrary::FolderAcl.where("title_id=#{self.title_id} AND folder_id=#{self.id}").destroy_all
  end
  ...
  #  acl_flag:制限種別（0 ：権限なし、 1：所属権限　2:ユーザー権限　9：管理者）
  # ==== 引数
  #  * なし
  # ==== 戻り値
  #  なし
  ...
  def child_count
    file_base  = Doclibrary::Doc
    file_cond  = "state!='preparation' and category1_id=#{self.id}"
    file_count = file_base.where(file_cond).count

    folder_base = Doclibrary::Folder
    folder_cond = "state!='preparation' and parent_id=#{self.id}"
    folder_count  = folder_base.where(folder_cond).count
  ...
  def readable_public_children(is_admin = false)
    item = Doclibrary::Folder.where(parent_id: id, title_id: title_id, state: 'public')
    sql = Condition.new
    sql.or do |c2|
    ...
    sql.or do |c2|
      ...
      c2.and 'doclibrary_folder_acls.acl_section_id', Core.user.user_group_parent_ids
    end
    sql.or do |d2|
        d2.and 'doclibrary_folder_acls.acl_flag', 1
        d2.and 'doclibrary_folder_acls.acl_section_id', 0
    end
    sql.or do |c2|
    ...
    item = item.where(sql.where)
              .joins('INNER JOIN doclibrary_folder_acls on doclibrary_folders.id = doclibrary_folder_acls.folder_id')
              .order('doclibrary_folders.sort_no')
              .select('DISTINCT doclibrary_folders.*')
  end
  ...
  def admin_user?(target_user_id = Core.user.id)
  ...
  # ==== 戻り値
  #  true:権限あり / false:権限無し
  def readable_user?(target_user_id = Core.user.id)
    target_user = System::User.find(target_user_id)
    return false if target_user.blank?
...
protected
  ...
  def self.get_child_without_admin_ids(parent_folder, without_admin_ids, is_parent_admin,
    ...
    user_group_ids = target_user.user_group_parent_ids
    ...
    unless is_admin
      ...
      if admins_ids.include?(target_user.id.to_s)
        is_admin = true
      end
    end
  ...
  def readable_user_check(target_user_id)
    ...
    reader_groups.each do |group|
      return true if group[1].to_i == 0 || group[2] == I18n.t("rumi.gwboard.no_limit.name")
      return true if user_group_ids.include?(group[1].to_i)
    end
