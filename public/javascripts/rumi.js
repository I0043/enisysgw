...
rumi.dragdrop = function(fileMoveAction, folderMoveAction) {
  ...
  jQuery('#sideList ul li a.draggable').draggable({
    opacity: 0.8,
    cursor: "move",
    helper: function(e) {
      ...
      elm_div.css('background', 'url(/images/icon/folder_y.png)');
    ...
    helper: function(e) {
      ...
      jQuery('table.docformTitle tbody tr').each(function() {
        if (jQuery(this).get(0) == target.get(0) || jQuery(this).find('input').is(':checked')) {
          ...
          elm_span.css('background', 'url(/images/icon/file.png)');
          elm_span.css('padding-left', '15px');
  ...
  jQuery('#sideList ul li a.droppable').droppable({
    hoverClass: "ui-hover",
 ...
 * @return {void}
 */
rumi.BlockKeyCode = function(evt) {
  evt = (evt) ? evt : event;
  var charCode=(evt.charCode) ? evt.charCode :
        ((evt.which) ? evt.which : evt.keyCode);
  if ((Number(charCode) >= 48 && Number(charCode) <= 111) 
          || (Number(charCode) >= 186 && Number(charCode) <= 244)
          || (Number(charCode) == 32)) {
    return false;
  } else {
    return true;
...
/**
 * Enterとキーボード入力を無効にするメソッド
 * @param {evt} event
 * @return {void}
 */
rumi.BlockEnterKeyCode = function(evt) {
  evt = (evt) ? evt : event;
  var charCode=(evt.charCode) ? evt.charCode :
        ((evt.which) ? evt.which : evt.keyCode);
  if ((Number(charCode) >= 48 && Number(charCode) <= 111) 
          || (Number(charCode) >= 186 && Number(charCode) <= 244)
          || (Number(charCode) == 32)
          || (Number(charCode) == 13) || (Number(charCode) == 3)) {
    return false;
  } else {
    return true;
  }
...
rumi.ui.SelectGroup.prototype.getHiddenItem_pre = function() {
  return jQuery(rumi.ui.idSelector(this.hidden_item_name+"_pre"));
};
...
rumi.ui.SelectGroup.prototype.updateSelected = function() {
  ...
  to.find("option").each(function(i, option) {
    ...
    record_name = record_name.replace(/^(\|\s)?\+\-*\s/, "");
  ...
  if (this.approval_hook) {
    ...
    this.getHiddenItem_pre().val(pre);
    name = this.hidden_item_name+"_pre"
    if($(name)){
      var user_pre = $(name);
      resize(user_pre);
    }
...
rumi.mailStateDefault = function() {
  jQuery(document).on('click', '.disabledLink', function(){
    alert('メール機能を利用できません。しばらくお待ちいただくか、メールアカウントの設定を確認してください。');
    return false
  });
}

rumi.mailStateOn = function() {
  jQuery('.MailLink , .MailSetting').removeClass('disabledLink');
  jQuery('.MailLink').css('cursor', 'pointer');
}
  ...
  if (url == '') {
    ...
    jQuery.ajax({
      ...
      success: function(data) {
        //メールリンクを有効にする。
        rumi.mailStateOn();
        rumi.unread.showMailCount(JSON.parse(data).total_count);
      },
      error: function(e) {
        // 未読件数が取得できなかった場合は 0 件とする。且つカーソルを変更しリンク無効とする。
        rumi.unread.showMailCount(0);
      }
    });
  }
};
...
rumi.unread.reminderRequest = function(url) {
  this.ajaxRequest(url,
    function(data) {
      jQuery('.infoContainer').html(data);
    },
    function(e) {
