...
module Gwboard::Controller::Common
  ...
  def gwbbs_select_status(params)
    ...
    when "DRAFT"
      ...
      unless @gw_admin
        str += " AND #{table_name}.section_code in (#{user_groups_code})"
      end
    when "RECOGNIZE"
      str  = "#{table_name}.state = 'recognize'"
      if @is_recognize_readable
      else
        str += " AND #{table_name}.section_code in (#{user_groups_code})" unless @gw_admin
      end
    when "PUBLISH"
  ...
  def gwfaq_select_status(params)
    ...
    when "DRAFT"
      ...
      str += " AND section_code = '#{Core.user_group.code}'" unless @gw_admin
    when "RECOGNIZE"
  ...
  def gwqa_select_status(params)
    ...
    when "DRAFT"
      str = "state = 'draft'"
      str += " AND (section_code = '#{Core.user_group.code}' or creater_id = '#{Core.user.code}')" unless @gw_admin  #管理者なら全記事対象
    when "RECOGNIZE"
      str = "state = 'recognize'"
    when "PUBLISH"
      str = "state = 'recognized'"
  ...
  def initialize_value_set
    ...
    params[:limit] = @title.default_limit.to_i unless is_integer(params[:limit])
    params[:limit] = @title.default_limit.to_i if params[:limit].blank?
    params[:limit] = @title.default_limit.to_i if params[:limit].to_i < 1
    params[:limit] = @title.default_limit.to_i if 100 < params[:limit].to_i
  ...
  def initialize_value_set_new_css
    ...
    unless params["url"].include?("doclibrary")
      ...
      f_path = Rails.root.join("public/#{css_path}")
    end
  ...
  def update_creater_editor
    ...
    if mode == 'create'
      ...
      @item.creater_admin = true if @gw_admin
      @item.creater_admin = false unless @gw_admin
      @item.editor_admin = true if @gw_admin          #1
      @item.editor_admin = false unless @gw_admin     #0
    else
  ...
  def users_collection(mode=nil)
    ...
    item = System::User
    users = item.joins(join).order('code').where(sql.where)
    users.each do |u|
      next if u == Core.user && Core.user.ldap != 0
      next unless @gw_admin if u.id == Core.user.id
      @users_collection << u
    end
