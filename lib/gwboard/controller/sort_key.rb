module Gwboard::Controller::SortKey
  ...
  def gwbbs_select_status(params)
    ...
    case params[:state]
    when "DRAFT"
      str = "state = 'draft'" if @gw_admin
      str = "state = 'draft' AND editor_id = '#{Core.user.code}' OR state = 'draft' AND editordivision_id = '#{Core.user_group.code}' AND editor_admin = 0" unless @gw_admin
    when "RECOGNIZE"
      str = "state = 'recognize'"
    when "PUBLISH"
