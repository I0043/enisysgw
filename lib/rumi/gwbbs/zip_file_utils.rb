...
# 本モジュールは、回覧板用zipファイル作成モジュールである。
# 添付ファイル一括ダウンロード機能のzipファイルを作成する。
module  Rumi::Gwbbs::ZipFileUtils
  # 一時ファイル格納フォルダパス
  TMP_FILE_PATH = Rails.root.join("tmp/gwbbs")
  ...
  def self.zip(filename, zip_data, options = {})
    ...
    begin
      ...
      Zip::File.open(filename, Zip::File::CREATE) {|zf|
        zip_data.each {|entry_name, src|
          # フォルダ・ファイル名の長さチェック
          #e_name = entry_name.split("/")
          #e_name.each do |e|
          #  if e.bytesize >
          #      Joruri.config.application['sys.max_file_name_length']
          ...
          if src.blank?
            # zipファイルへフォルダのみ作成
            zf.mkdir(encode_path(entry_name, options[:fs_encoding]))
          else
            # 添付ファイルの存在チェック
            raise Errno::ENOENT unless File.exist?(src)

            # zipファイルへフォルダと添付ファイルを作成
            zf.add(encode_path(entry_name, options[:fs_encoding]), src)
          end
        }
      }
    rescue Errno::ENOENT
      raise I18n.t('rumi.gwbbs.message.attached_file_not_found')
    rescue Errno::ENAMETOOLONG
      raise I18n.t('rumi.gwbbs.message.file_name_too_long')
    rescue
      raise I18n.t('rumi.gwbbs.message.cannot_create_zip_file')
    end
  end
