...
module System::Model::Role
  ...
  def self.get_raw(class_id, uid, table_name, priv_name)
    ...
    when 1
      ...
      gid_wrk = System::UsersGroup.where(gid_cond).order("job_order").first
    ...
    uid = uid.to_i
    gid = gid.to_i
    grp_obj = System::Group.find_by(id: gid)
    gid_parents_raw = grp_obj.parent_tree
    gid_parents = gid_parents_raw.collect{|x| x.id}
    roles = System::Role.where(table_name: table_name, priv_name: priv_name).order('idx')
  ...
  def self.get_roles(uid = Core.user.id)
    user_group = System::UsersGroup.where("user_id=#{uid} and end_at is null ").order("job_order").first
    ...
    roles = System::Role.where(cond).order(order)
